...
remove_nonsense_times <- function(x, y) { fifelse(!is.na(x), y, NA) }
...
compute_event_times_from_delays <- function(delays_df) { # Note: this function assumed non-existent delays are NA's.
  ...
  times_df <- delays_df %>%
    ...
    mutate(
      time_obs_case = find_first_time(time_bg_test, time_bg_hosp_test, time_severe_hosp_test, time_seek_test),
      time_severe_hosp_obs = remove_nonsense_times(time_severe_hosp_test, time_severe_hosp),
      time_bg_hosp_obs = remove_nonsense_times(time_bg_hosp_test, time_bg_hosp),
