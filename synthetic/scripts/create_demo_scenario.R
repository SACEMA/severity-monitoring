...
scenario_number <- sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(.args[3])) %>%
...
params <- param_table %>% filter(scenario == scenario_number)
...
#Save the combined and individual ts
save(
  list = c("ts_combined", "dd_strain_1", "dd_strain_2"),
  file = tail(.args, 1)
)
