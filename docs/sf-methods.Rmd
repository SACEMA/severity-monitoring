...
In this section, we define the model used as part of our surveillance framework and the approach taken to use this model to evaluate if the underlying relationship between primary and secondary indicators (i.e test positive cases and deaths) has recently changed compared to a baseline period. Note that the model we define here has been used successfully, via its implementation in the `EpiNow2` R package [@EpiNow2], to forecast local area hospital transmissions [@Meakin2022-hw], to forecast COVID-19 notified deaths in Germany and Poland [@Bosse2021-jk], and to forecast deaths and hosptial admissions as part of the ECDC forecasting hub project [@ECDC-hub]. Refer to these references for a detailed evaluation of its use for out of sample forecasting.  
...
This model is based on a discrete convolution of primary cases, scaled based on the fraction (here described as the secondary fraction but depending on application potentially being the case fataility ratio, case hospitalisation ratio, or the hospitalisation fatality ratio), and a delay distribution that is assumed to follow a daily discretised log normal distribution.
...
We generalise this simple model to be both the incidence case, which is as described, and the prevalence case (for example hospital admissions and occupancy) where the secondary notifications can be thought of as depending on secondary notifications from the previous timestep, plus scaled current primary notications, and minus scaled historic primary notifications weighted by some delay distribution. 
...
$\alpha$ and $\xi$ are defined as the secondary fraction and delay from primary to secondary notification (or delay from secondary notification to recovery etc in the prevalence case).
...
Here we assume a negative binomial observation model (though our implementation also supports a Poisson observation model) in order to capture potential reporting overdispersion $\phi$ and adjust expected counts using an optional day of the week effect based on a simplex $\omega_{(t \mod 7)}$ (i.e sums to 7 so the total effect over a week is balanced).
This gives the following observation process. 

\begin{equation} 
    S_{t} \sim \mathrm{NB}\left(\hat{S}_t,  \phi \right)
\end{equation}

# Model priors

We define weakly informative default priors for all parameters based on our subject area knowledge. These are as follows,
...
Note that we choose $\mu$ and $\sigma$ such that the discretised log normal distribution has a mean of 10 and a standard deviation of 5 on the natural scale. We also set maximum delay $D$ to 30. Here $\alpha$, $\sigma$, and $\phi$ are truncated to be greater than 0.

# Defining a surveillance indicator

We now use the convolution model described in the last section to construct and indicator for whether or not the relationship between primary and secondary notifcations is changing over time. 
...
Under an assumption of no change this approach should provide reasonable predictive accuracy and so if it does not we may be able to conclude that there has been an underlying change in the relationship between the two indicators of interest.
In order to account for the expected in sample performance of our model (i.e its potential misspecification etc.) we normalise performance of predictions from our baseline time period using predictions from a model fit to the time period of interest. 
...
We build then build on this in later sections to develop a decision criteria for use as a surveillance tool. More detail is provided in the following sections.
...
We calculate the CRPS for predictions from both the baseline and target model on the target window of interest.
